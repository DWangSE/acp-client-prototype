...
export class AutoStrategy implements AuthStrategy {
  ...
  async execute(authMethods: any[], verbose?: boolean): Promise<AuthCredential | null> {
    ...
    const terminalMethod = authMethods.find(m => 
        m.type === "terminal" || (m._meta && m._meta["terminal-auth"])
    );
    ...
    if (terminalMethod) {
        ...
        if (command) {
            ...
            if (verbose) console.log(`[Auth:Auto] Found terminal-auth for ${terminalMethod.id}. Executing: ${command} ${args.join(" ")}`);
            console.log("\n══════════════════════════════════════════════════════════════");
    ...
    if (verbose) console.log("[Auth:Auto] No environment match or terminal-auth found, falling back to interactive selection");
